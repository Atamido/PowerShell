...
function Start-PEHTTPServer {
    ...
    While ($SharedVariables['WebServerActive']) {
        ...
        $Jobs | Where {$_.Handle.IsCompleted} | ForEach {
        ...
        $RemoveJobs | Where {$_.Handle.IsCompleted} | ForEach {
            $Jobs.Remove($_)
        }
    ...
    [Void]$Listener.Stop()
    while (@($Jobs | Where {-not ($_.Handle.IsCompleted)}).Count -ne 0) {
        Write-Verbose "Waiting on $(@($Jobs | Where {-not ($_.Handle.IsCompleted)}).Count) threads to complete"
        Start-Sleep -Milliseconds 250
    }
