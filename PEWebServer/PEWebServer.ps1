...
function New-RunspaceScriptBlock {
    ...
    [ScriptBlock]$ScriptBlock = {
        ...
        try {
            Write-Verbose "New-RunspaceScriptBlock: Retrieving stream from $($ClientIP) in thread $($ThreadId) at $((Get-Date).ToString('HH:mm:ss.fff'))"
            $Stream = $TcpClient.GetStream()
            Write-Verbose "New-RunspaceScriptBlock: Retrieved stream from $($ClientIP) in thread $($ThreadId) at $((Get-Date).ToString('HH:mm:ss.fff'))"
        }
        catch {
            Write-Error $Error[0]
            Write-Error "New-RunspaceScriptBlock: Unable to get stream with `$TcpClient.GetStream() from $($ClientIP) in thread $($ThreadId) at $((Get-Date).ToString('HH:mm:ss.fff'))"
            [Void]$Stream.Flush()
            [Void]$Stream.Dispose()
        ...
        Write-Verbose "New-RunspaceScriptBlock: Created HttpListenerRequest from $($ClientIP) in thread $($ThreadId) at $((Get-Date).ToString('HH:mm:ss.fff'))"
        ...
        if ($HttpListenerRequest.IsValid) {
            ...
            if ($HttpListenerRequest.HTTPBytes.Count -ne 0) {
                Write-Warning "New-RunspaceScriptBlock: Invalid HTTP Request from $($ClientIP) in thread $($ThreadId) at $((Get-Date).ToString('HH:mm:ss.fff')) `n$($HttpListenerRequest | Out-String)"
            }
            $HTTPResponseBytes = New-HTTPResponseBytes
            Write-Verbose "New-RunspaceScriptBlock: Echoing $($HTTPResponseBytes.count) bytes to $($ClientIP) in thread $($ThreadId) at $((Get-Date).ToString('HH:mm:ss.fff'))"
            $Stream.Write($HTTPResponseBytes, 0, $HTTPResponseBytes.length)
        }

        Write-Verbose "New-RunspaceScriptBlock: Closing session to $($ClientIP) in thread $($ThreadId) at $((Get-Date).ToString('HH:mm:ss.fff'))"
        [Void]$Stream.Flush()
        [Void]$Stream.Dispose()
        [Void]$TCPClient.Close()
        ...
        [System.GC]::Collect()
        Write-Verbose "New-RunspaceScriptBlock: Closed session to $($ClientIP) in thread $($ThreadId) at $(Get-Date -Format 'o')"
    }
